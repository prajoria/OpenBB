...
def test_extension_map():
    ...
    # The "extra files present" assertion is CI-only: it guards against
    # accidentally committing files generated by `openbb.build()`. In
    # any environment where the platform has actually been built (local
    # dev, editable install, docker harness), those files are expected
    # to be present. Skip unless the caller explicitly opts into strict
    # mode via ``OPENBB_STRICT_EXTENSION_MAP=1`` (CI sets this).
    if contents and os.environ.get("OPENBB_STRICT_EXTENSION_MAP") != "1":
        pytest.skip(
            "Package directory contains generated files (expected after "
            "openbb.build() or editable install). This test enforces "
            "'no generated files committed to the repo' — set "
            "OPENBB_STRICT_EXTENSION_MAP=1 in CI to enforce. "
