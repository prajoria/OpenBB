"""Cached cash_flow model for FMP with dedicated database persistence."""
...
from openbb_fmp.models.cash_flow import (
...
CASH_FLOW_TTL_DAYS = 1


class FMPCachedCashFlowStatementFetcher(FMPCashFlowStatementFetcher):
    """FMP Cached Cash Flow Statement Fetcher with dedicated database persistence."""
    ...
    async def aextract_data(
        ...
        **kwargs: Any,
        ...
        """Extract cash flow statement data with database persistence."""
        ...
        for symbol in symbols:
            cached = _get_cached_cash_flow(symbol, query)
            if cached:
    ...
    def transform_data(
        ...
        **kwargs: Any,
        ...
        return FMPCashFlowStatementFetcher.transform_data(query, data, **kwargs)
...
def _get_cached_cash_flow(
    symbol: str,
    query_params: FMPCashFlowStatementQueryParams,
) -> list[dict[str, Any]]:
    """Read recent cash flow statement data from cache."""
    freshness_cutoff = datetime.now() - timedelta(days=CASH_FLOW_TTL_DAYS)
    query = """
    SELECT data_json
    FROM cash_flow
    WHERE symbol = %s
