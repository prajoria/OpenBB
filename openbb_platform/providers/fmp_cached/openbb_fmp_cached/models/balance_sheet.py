"""Cached balance_sheet model for FMP with dedicated database persistence."""
...
from openbb_fmp.models.balance_sheet import (
    FMPBalanceSheetData,
...
BALANCE_SHEET_TTL_DAYS = 1


class FMPCachedBalanceSheetFetcher(FMPBalanceSheetFetcher):
    """FMP Cached Balance Sheet Fetcher with dedicated database persistence."""
    ...
    async def aextract_data(
        ...
        **kwargs: Any,
        ...
        """Extract balance sheet data with database persistence."""
        ...
        for symbol in symbols:
            cached = _get_cached_balance_sheet(symbol, query)
            if cached:
    ...
    def transform_data(
        ...
        **kwargs: Any,
        ...
        return FMPBalanceSheetFetcher.transform_data(query, data, **kwargs)
...
def _get_cached_balance_sheet(
    symbol: str,
    query_params: FMPBalanceSheetQueryParams,
) -> list[dict[str, Any]]:
    """Read recent balance sheet data from cache."""
    freshness_cutoff = datetime.now() - timedelta(days=BALANCE_SHEET_TTL_DAYS)
    query = """
    SELECT data_json
    FROM balance_sheet
    WHERE symbol = %s
