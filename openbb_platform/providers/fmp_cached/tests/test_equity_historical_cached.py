...
# Test credentials
test_credentials = {
    "fmp_cached_api_key": "test_api_key_123"
}
...
def setup_test_tables(setup_test_database):
    ...
    try:
        execute_query("DELETE FROM equity_historical WHERE symbol = %s OR symbol LIKE %s", ('AAPL', 'TEST%'))
    except Exception:
...
class TestEquityHistoricalCachedFetcher:
    ...
    def test_transform_data(self, sample_query, sample_fmp_data):
        ...
        result = FMPCachedEquityHistoricalFetcher.transform_data(sample_query, sample_fmp_data)
...
class TestGapDetectionLogic:
    ...
    def test_analyze_cache_gaps_complete_cache(self, sample_query, setup_test_tables):
        ...
        cache_rows = [
            ("AAPL", date(2024, 1, 2), 185.64, 186.95, 185.0, 185.64, 54120000, -1.91, -1.02, 185.82, interval_type, adjustment_type),
            ("AAPL", date(2024, 1, 3), 184.22, 185.88, 183.43, 184.25, 58953400, -1.39, -0.75, 184.64, interval_type, adjustment_type),
            ("AAPL", date(2024, 1, 4), 182.09, 182.76, 180.17, 181.91, 81235900, -2.34, -1.27, 181.47, interval_type, adjustment_type),
            ("AAPL", date(2024, 1, 5), 181.99, 182.76, 180.17, 181.18, 62379700, -0.81, -0.44, 181.52, interval_type, adjustment_type),
            ("AAPL", date(2024, 1, 8), 182.09, 185.60, 181.50, 185.56, 59144500, 3.47, 1.91, 183.69, interval_type, adjustment_type),
            ("AAPL", date(2024, 1, 9), 183.92, 185.15, 182.73, 185.14, 42841809, 1.22, 0.66, 184.24, interval_type, adjustment_type),
            ("AAPL", date(2024, 1, 10), 184.35, 186.40, 183.92, 186.19, 46792908, 1.84, 0.99, 185.22, interval_type, adjustment_type),
        ]
    ...
    def test_analyze_cache_gaps_partial_cache(self, sample_query, setup_test_tables):
        ...
        cache_rows = [
            ("AAPL", date(2024, 1, 2), 185.64, 186.95, 185.0, 185.64, 54120000, -1.91, -1.02, 185.82, interval_type, adjustment_type),
            ("AAPL", date(2024, 1, 5), 182.09, 182.76, 180.17, 181.91, 81235900, -2.34, -1.27, 181.47, interval_type, adjustment_type),
        ]
...
class TestMissingRangeDetection:
    ...
    def test_detect_missing_ranges_intraday_empty(self):
        ...
        missing_ranges = _detect_missing_ranges(start_date, end_date, cached_dates, "1h")
...
class TestDatabaseOperations:
    ...
    def test_store_in_database_cache(self, sample_query, sample_fmp_data, setup_test_tables):
        ...
        count_query = "SELECT COUNT(*) as count FROM equity_historical WHERE symbol = %s"
    ...
    def test_store_in_database_cache_invalid_dates(self, sample_query, setup_test_tables):
        """Test handling of invalid date formats."""
        from openbb_fmp_cached.utils.database import execute_query
        
        invalid_data = [
            {"symbol": "AAPL", "date": "2024-01-02", "close": 152.0,  "open": 150.0, "high": 153.0, "low": 149.0, "volume": 1000000},
        ]
...
class TestMultiSymbolSupport:
    ...
    def test_mixed_cache_states(self, setup_test_tables):
        ...
        ON DUPLICATE KEY UPDATE cached_at = NOW()
        """
        execute_query(insert_query, ('AAPL', date(2024, 1, 2), 185.0, interval_type, adjustment_type))
...
class TestErrorHandling:
    ...
    def test_database_connection_resilience(self, setup_test_tables):
        ...
        try:
            ...
            assert "nonexistent_table" in str(e).lower() or "doesn't exist" in str(e).lower()
    ...
    def test_cache_analysis_with_no_data(self, setup_test_tables):
        ...
        execute_query("DELETE FROM equity_historical WHERE symbol = %s", ('TESTNODATA',))
...
class TestPerformanceOptimizations:
    ...
    def test_gap_detection_efficiency(self, setup_test_tables):
        ...
        execute_query(insert_query, ('PERF_TEST', date(2024, 1, 2), 100.0, interval_type, adjustment_type))
        execute_query(insert_query, ('PERF_TEST', date(2024, 1, 10), 105.0, interval_type, adjustment_type))
...
class TestHolidayGapFilling:
    ...
    def test_fill_weekend_gap_with_previous_close(self, setup_test_tables):
        ...
        sat_record = next((d for d in filled_records if d['date'] == '2024-01-06'), None)
    ...
    def test_fill_holiday_gap_with_next_open(self, setup_test_tables):
        ...
        christmas_record = next((d for d in filled_data if d['date'] == '2024-12-25'), None)
