...
def _build_ssga_fixture_workbook(path: Path) -> None:
    ...
    ws.append(
        ["APPLE INC", "AAPL", "037833100", "2046251", 12.613907, "-", 54995918.0, "USD"]
    )
    ws.append(
        [
            "MICROSOFT CORP",
...
def test_parse_ssga_xlsx_emits_weight_as_percent_not_fraction(ssga_fixture_path):
    """SSGA writes weight as 14.79788 (%); parser passes it through unchanged.

    UPDATED for #512: the previous version of this test asserted parser
    output was a *fraction* (0.14 < w < 0.15), which was correct given
    the previous `if weight > 1: weight /= 100` heuristic. But that
    heuristic was actually a bug — it silently kept sub-1% holdings as
    fractional-looking-numbers-that-are-really-percentages, producing
    SPY total_weight = 57 instead of 1. Discovered by #512's live
    fixtures. The correct behavior is: parser emits percentages
    UNCHANGED (matching FMP's raw API shape); the FMPEtfHoldingsData
    validator normalizes percent → fraction at model construction.
    """
    from openbb_fmp_cached.models.etf_holdings_issuer import _parse_ssga_xlsx

    rows = _parse_ssga_xlsx(ssga_fixture_path.read_bytes(), ticker="XLK")
    nvda = next(r for r in rows if r["symbol"] == "NVDA")
    # Percent form: NVDA in XLK ≈ 14.79%
    assert 14.0 < nvda["weight"] < 15.5
