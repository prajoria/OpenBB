"""Tests for ``openbb_pine.stdlib.ta.roc`` — S-bead OpenBBTechnical-0e9.5.37 (Wave 5B-4).

ROC = 100 * (source - source[length]) / source[length]. Divide-by-zero
posture is documented at the bridge: PyneCore does not guard the
divisor, so ``source[length] == 0.0`` raises Python ``ZeroDivisionError``
— out of scope for the bridge to widen.
"""
...
class TestBridgeExists:
    def test_bridge_importable(self) -> None:
        ...
        assert hasattr(ta, "roc"), "ta.roc bridge missing"
        assert callable(ta.roc)

    def test_bridge_in_module_all(self) -> None:
        from openbb_pine.stdlib import ta

        assert "roc" in ta.__all__
...
class TestBridgeDelegates:
    def test_calls_pynecore_ta_roc(self) -> None:
        from openbb_pine.stdlib import ta as _bridge

        with patch("openbb_pine.stdlib.ta._pyne_ta.roc") as mock_fn:
            mock_fn.return_value = 5.0
            result = _bridge.roc("CLOSE", 3)
            mock_fn.assert_called_once_with("CLOSE", 3)
            assert result == 5.0


class TestCoverageManifest:
    def test_ta_roc_in_implemented_manifest(self) -> None:
        from openbb_pine import _coverage_manifest

        assert "ta.roc" in _coverage_manifest.BUILTINS_IMPLEMENTED
...
class TestConformancePairPresent:
    ...
    def test_pine_fixture_exists(self) -> None:
        assert (self._ROOT / "ta_roc.pine").is_file()

    def test_csv_fixture_exists_and_parses(self) -> None:
        csv_path = self._ROOT / "ta_roc.csv"
        assert csv_path.is_file()
        with csv_path.open(encoding="utf-8") as fh:
            rows = list(csv.DictReader(fh))
        assert "roc3" in rows[0]
        assert len(rows) >= 5
