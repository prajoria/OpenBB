...
2. Paste the entire contents of `sma_crossover.pine` into the editor.
...
7. Export three CSVs from the Strategy Tester panel:
   - **List of Trades → Export CSV** → save as `sma_crossover.trades.csv`
   - **Performance Summary → Export CSV** (or the KPI table) → save as
     `sma_crossover.stats.csv`
   - **Equity curve** (may require right-click → Export or a plugin) →
     save as `sma_crossover.equity.csv` with columns
...
The Pine source in `sma_crossover.pine` was authored from scratch
