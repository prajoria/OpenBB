# How to export `breakout_atr_trail` reference CSVs from TradingView
...
This fixture ships the Pine v6 strategy source (`breakout_atr_trail.pine`)
...
2. Paste the entire contents of `breakout_atr_trail.pine` into the editor.
...
7. Export three CSVs from the Strategy Tester panel:
   - **List of Trades → Export CSV** → save as `breakout_atr_trail.trades.csv`
   - **Performance Summary → Export CSV** (or the KPI table) → save as
     `breakout_atr_trail.stats.csv`
   - **Equity curve** (may require right-click → Export or a plugin) →
     save as `breakout_atr_trail.equity.csv` with columns
...
**Landed in bd-0ru2**: export a `breakout_atr_trail.bars.csv` alongside the three
...
3. Save as `breakout_atr_trail.bars.csv` in this directory.
...
The Pine source in `breakout_atr_trail.pine` was authored from scratch
