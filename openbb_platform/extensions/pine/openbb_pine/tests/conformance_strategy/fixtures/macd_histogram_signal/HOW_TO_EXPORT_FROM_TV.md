# How to export `macd_histogram_signal` reference CSVs from TradingView
...
This fixture ships the Pine v6 strategy source (`macd_histogram_signal.pine`)
...
2. Paste the entire contents of `macd_histogram_signal.pine` into the editor.
...
7. Export three CSVs from the Strategy Tester panel:
   - **List of Trades → Export CSV** → save as `macd_histogram_signal.trades.csv`
   - **Performance Summary → Export CSV** (or the KPI table) → save as
     `macd_histogram_signal.stats.csv`
   - **Equity curve** (may require right-click → Export or a plugin) →
     save as `macd_histogram_signal.equity.csv` with columns
...
**Landed in bd-0ru2**: export a `macd_histogram_signal.bars.csv` alongside the three
...
3. Save as `macd_histogram_signal.bars.csv` in this directory.
...
The Pine source in `macd_histogram_signal.pine` was authored from scratch
