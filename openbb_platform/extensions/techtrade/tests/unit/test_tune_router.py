...
def _config_a() -> IndicatorConfig:
    """Return a non-default tuned candidate distinct from DEFAULT_CONFIG."""
...
def _setup_tuned_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setattr(
        "openbb_techtrade.tuning.tune_router._require_tuneta",
        lambda: None,
    )
...
def _fake_validate(verdict: str):
    ...
    async def _validate(
        plan, *, method="wfo", thresholds=None, horizon_years=5, provider=None
    ):
...
def test_robust_verdict_persists(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        "openbb_techtrade.tuning.tune_router.pool_sector_ohlcv",
        _fake_pool,
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "openbb_techtrade.tuning.tune_router.validate_plan",
        fake_validate,
    )
...
def test_robust_dry_run_never_persists(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    """Snapshot computation must not mutate the live tuned-defaults file."""
    ...
    assert "persistence disabled" in report.reason
...
def test_no_op_tune_does_not_persist(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    fake_validate, _ = _fake_validate(
        "robust"
    )  # even a robust verdict shouldn't write a no-op.
    monkeypatch.setattr(
        "openbb_techtrade.tuning.tune_router.validate_plan",
        fake_validate,
    )
...
def test_loop_over_mixed_verdicts_does_not_raise(
    ...
    def fake_report_factory():
        return _fake_validate(next(verdicts))
...
def test_override_visible_in_validate_fold(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
...
def test_tune_forwards_trials_and_early_stop(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
