...
21. [Technical Review & Grounded Feedback](#21-technical-review--grounded-feedback)
22. [Recommendations — From "Widget Host" to Full-Fledged Trading Terminal](#22-recommendations--from-widget-host-to-full-fledged-trading-terminal)
23. [Appendix A — Widget × Panel Mapping](#appendix-a--widget--panel-mapping)
24. [Appendix B — VS Code API Surface Used](#appendix-b--vs-code-api-surface-used)
...
> ⚠️ **REVIEW NOTE (backend topology is not a single port-8000 app).**
> The diagram collapses three *different* backend surfaces that exist in
> the codebase today into one `uvicorn :8000`. Grounded reality:
> - **Core REST API** — `openbb_core.api.rest_api:app`. The desktop SDK
>   client ([desktop/src/pi/sdk/client.ts](../../desktop/src/pi/sdk/client.ts)) defaults to
>   `http://127.0.0.1:8000`, **but** the Tauri app actually spawns
>   `openbb-api --host 127.0.0.1 --port 6900`
>   ([desktop/src-tauri/src/tauri_handlers/startup.rs](../../desktop/src-tauri/src/tauri_handlers/startup.rs)), and MCP on `:8001`.
> - **Portfolio-Intel widget backend** — a *separate* FastAPI app
>   ([widget_backend/main.py](../../openbb_platform/extensions/portfolio_intel/openbb_portfolio_intel/widget_backend/main.py))
>   that serves `/widgets.json`, `/apps.json`, and the `pi/*` endpoints on
>   **port 6120**, with CORS locked to `https://pro.openbb.co` and bearer-token
>   auth. This is the app that actually hosts the 60+ `pi_*` routes.
> **Action:** pick ONE canonical spawn (recommended: the same
> `openbb-api` invocation the desktop app already uses, so widget parity
> G9 is real and not aspirational), pin the port in one setting, and make
> the diagram reflect the merged-router reality. See §21.1.
...
> ⚠️ **REVIEW NOTE (the "reuse 100% of existing widgets" premise is not yet true).**
> The desktop React registry ([desktop/src/pi/sdk/registry.ts](../../desktop/src/pi/sdk/registry.ts)) and
> `WidgetCanvas` ([desktop/src/pi/canvas/WidgetCanvas.tsx](../../desktop/src/pi/canvas/WidgetCanvas.tsx)) are real and
> production-quality, but **only one widget is currently registered** —
> `hello` ([desktop/src/pi/widgets/](../../desktop/src/pi/widgets/) contains `hello.tsx` + `index.ts`).
> The landing route ([desktop/src/routes/pi/index.tsx](../../desktop/src/routes/pi/index.tsx)) says so explicitly:
> *"when the widget backlog lands (#529-#577), individual dashboards …
> will each get their own route"* and renders `slots={[{ widgetId: "hello" }]}`.
> The 60+ widgets listed in §10 / Appendix A are **Python backend
> endpoints** in [widget_backend/widgets.json](../../openbb_platform/extensions/portfolio_intel/openbb_portfolio_intel/widget_backend/widgets.json), rendered today by the
> **OpenBB Workspace cloud renderer** (pro.openbb.co) — not by React
> components in this bundle. **This is the single biggest scope gap in the
> PRD.** Either (A) build a generic `widgets.json`-driven renderer in the
> webview (reuses all 60+ endpoints, ~1 renderer to build), or (B) hand-
> write 60+ React components (backlog #529-#577). The roadmap in §18 must
> pick one and cost it. See §21.2 for the recommended path.
...
> ⚠️ **REVIEW NOTE (`/healthz` does not exist; use the real readiness gate).**
> There is no `/healthz` route in the platform. The existing browser test
> harness uses **`GET /widgets.json`** as its readiness gate
> ([tools/browser_test_harness/…/standalone_driver.py](../../openbb_platform/tools/browser_test_harness/src/openbb_browser_test_harness/drivers/standalone_driver.py)), and the widget
> backend exposes `GET /pi/health/providers` (markdown) for provider tier
> status. **Action:** replace every `/healthz` reference (§7 diagram, §8.4,
> §14.1) with `/widgets.json` for liveness and `/pi/health/providers` for
> provider health. Also align the spawn command with the desktop app —
> `openbb-api` (which merges the widget routers), not a bare
> `uvicorn openbb_core.api.rest_api:app`, otherwise the `pi_*`/`tt_*`
> widget endpoints won't be mounted (§21.1).

**Start sequence:**
1. Check `GET http://127.0.0.1:{port}/widgets.json` — if 200, mark running (already started externally or from previous session).
...
> ⚠️ **REVIEW NOTE (two auth/CORS realities the PRD omits).**
> "No new auth" is true for *provider* keys, but the **widget backend has
> its own request auth**: `require_auth()` expects a bearer token
> (`PI_WIDGET_BACKEND_TOKEN`) or an explicit `loopback-dev` opt-out
> ([widget_backend/_shared.py](../../openbb_platform/extensions/portfolio_intel/openbb_portfolio_intel/widget_backend/_shared.py)), and **CORS is hard-locked to
> `https://pro.openbb.co`** ([widget_backend/_app.py](../../openbb_platform/extensions/portfolio_intel/openbb_portfolio_intel/widget_backend/_app.py)). A VS Code webview
> ships an opaque `vscode-webview://` origin, so browser `fetch` to that
> backend will be **rejected by CORS**, and every authed endpoint needs
> the bearer token injected. **Action (both must be done, see §21.3):**
> (1) add the webview origin to the allowed-origins list (or run the
> extension against the `openbb-api`/core surface, which is loopback-open),
> and (2) have the extension host inject the bearer token into the webview
> so widget `fetch`/`EventSource` calls carry `Authorization`.
...
## 21. Technical Review & Grounded Feedback

> **Reviewer:** Platform/agent review pass, 2026-08-03. This section was
> added on top of the original proposal. It cross-checks every load-bearing
> claim against the code in this repository and records concrete
> corrections. Inline `⚠️ REVIEW NOTE` callouts elsewhere in the document
> point back here. **Nothing below changes the product vision — the vision
> is sound. These are the facts the build plan must be re-based on so the
> estimates are honest and the first sprint doesn't hit a wall.**

### 21.0 Verdict

The PRD is **well-structured, correctly scoped as a shell-not-engine, and
strategically right** — a developer-resident terminal is a genuinely
differentiated surface and the VS Code API choices (Webview-per-tab,
postMessage bus, theme-token bridge, hover provider, notebook watcher) are
all real, non-experimental, and appropriate. **Approve the direction.**

However, four premises are stated as *done* when the code shows them as
*to-be-built*, and correcting them roughly **doubles the honest effort**
and inserts a mandatory Phase 0. None is a blocker; all are addressable.

### 21.1 Backend topology & lifecycle (correct the port/spawn story)

| Claim in PRD | Grounded reality | Fix |
|---|---|---|
| One `uvicorn :8000` serves widgets + data + SSE (§7 diagram) | Three surfaces exist: core REST API (SDK client defaults to `:8000`, but Tauri spawns `openbb-api … :6900`), the **PI widget backend on `:6120`**, and MCP on `:8001` | Pick one canonical backend. **Reuse the desktop app's exact spawn** (`openbb-api …`) so widget routers are merged and parity (G9) is real |
| Spawn `uvicorn openbb_core.api.rest_api:app` (§14.1) | Bare core app does **not** mount the `pi_*`/`tt_*` widget endpoints; those are merged by `openbb-api` (platform_api) / live in the widget_backend | Spawn `openbb-api` (or the widget backend), not the bare core app |
| Health via `GET /healthz` (§7, §8.4, §14.1) | **No `/healthz` route exists.** Harness readiness gate is `GET /widgets.json`; provider health is `GET /pi/health/providers` | Use `/widgets.json` for liveness, `/pi/health/providers` for provider status |

**Evidence:** [desktop/src/pi/sdk/client.ts](../../desktop/src/pi/sdk/client.ts) (`_defaultBase = "http://127.0.0.1:8000"`),
[desktop/src-tauri/src/tauri_handlers/startup.rs](../../desktop/src-tauri/src/tauri_handlers/startup.rs) (`openbb-api … --port 6900`, `openbb-mcp … --port 8001`),
[widget_backend/main.py](../../openbb_platform/extensions/portfolio_intel/openbb_portfolio_intel/widget_backend/main.py) (`uvicorn … --port 6120`),
[standalone_driver.py](../../openbb_platform/tools/browser_test_harness/src/openbb_browser_test_harness/drivers/standalone_driver.py) (readiness on `/widgets.json`),
[widgets_endpoints.py](../../openbb_platform/extensions/portfolio_intel/openbb_portfolio_intel/widget_backend/widgets_endpoints.py) (`@app.get("/pi/health/providers")`).

### 21.2 Widget parity is the real cost — decide the rendering model first

**The 60+ widget rows in §10/Appendix A are backend endpoints, not React
components.** The desktop `WidgetCanvas`/registry is a solid scaffold with
exactly **one** registered widget (`hello`), and the landing route says the
rest are backlog #529–#577. The PRD's G9 ("100% widget parity with the
`desktop/` Tauri app") is therefore trivially *true today* (both have one
widget) and *very expensive tomorrow* (both need 60+). Two honest paths:

- **Path A — Generic `widgets.json`-driven renderer (recommended).** Build
  **one** webview renderer that reads `widgets.json` and renders by declared
  `type` (`table`, `chart`, `markdown`, `metric`, `note`, …), fetching each
  widget's endpoint. This reuses **all 60+ existing backend endpoints
  immediately** and is how OpenBB Workspace itself works. Cost: one robust
  renderer + a type-to-component map, not 60 bespoke components. This also
  future-proofs new widgets (register a backend route → it appears, no
  frontend PR).
- **Path B — Hand-written React components (registry path).** Each widget is
  a `registerWidget(...)` component under `desktop/src/pi/widgets/`. Higher
  fidelity per widget, but it *is* backlog #529–#577 — 60+ components — and
  must be sequenced before "full parity" can be claimed.

**Recommendation:** ship **Path A** for breadth (all widgets available fast),
and selectively upgrade the highest-value widgets (charting, blotter,
scan table, risk dashboard) to **Path B** bespoke components where the
generic renderer isn't rich enough. Make this an explicit **Phase 0
decision** and reflect it in §18; the current roadmap silently assumes the
components already exist.

### 21.3 Webview ↔ backend security (CORS + token) is a Phase-1 gate, not a footnote

The widget backend **locks CORS to `https://pro.openbb.co`** and requires a
**bearer token** (`PI_WIDGET_BACKEND_TOKEN`, or `loopback-dev` opt-out).
A VS Code webview's origin is `vscode-webview://…` (opaque), so:

1. **CORS will reject webview `fetch`/`EventSource`** to `:6120` until the
   webview origin is added to `_ALLOWED_ORIGINS`, **or** the extension
   targets the loopback-open `openbb-api` surface instead.
2. **Every authed endpoint needs the token.** The extension host must read
   the token (from `user_settings.json` / env) and inject it into the
   webview so widget requests carry `Authorization: Bearer …`. The webview
   CSP `connect-src` (§17.3) is necessary but **not sufficient** — CSP is
   client-side; CORS + auth are server-side and are the actual blockers.

**Fix:** add an explicit Phase-1 spike ("prove one authed widget renders
end-to-end in a webview against the chosen backend") *before* building the
tree-views and layouts. This is R1 in the risk table but under-rated — it is
**High likelihood, High impact**, not "Medium/High".

### 21.4 Smaller corrections & confirmations

| # | Item | Status | Note |
|---|------|--------|------|
| C1 | `WidgetCanvas`, registry, per-slot error boundary, PAPER badge, `fixture` prop | ✅ **Real** | Matches §9/§17.4 exactly — good foundation ([WidgetCanvas.tsx](../../desktop/src/pi/canvas/WidgetCanvas.tsx)) |
| C2 | Fixture-mode preview route | ✅ **Real** | [desktop/src/routes/pi/preview/$widgetId.tsx](../../desktop/src/routes/pi/preview/$widgetId.tsx) already does single-widget fixture preview — the `openbb.previewWidget` command (§12) can reuse it |
| C3 | `VITE_OPENBB_API_BASE` injection | ✅ **Real & reusable** | §14.2's `window.__OPENBB_API_BASE__` should map to the existing `VITE_OPENBB_API_BASE` env the SDK client already reads, not a new global |
| C4 | Notebook variable watcher via `NotebookDocument` events (§8.6) | ⚠️ **Feasibility caveat** | VS Code does not expose kernel *variable values* through `NotebookDocument` text events reliably; watching source text for `symbol = "AAPL"` is a heuristic (works), but "kernel variable assignment" implies runtime introspection that needs the Jupyter extension's variable API or a kernel hook. Reword to "source-text heuristic" or scope to the Jupyter variables API explicitly |
| C5 | Symbol validation endpoint `GET /api/v1/equity/search` (§13.3) | ⚠️ **Verify path** | Confirm the exact route on the chosen backend surface before coding; the widget backend namespaces under `/pi/*`, the core API under `/api/v1/*` — they are different servers |
| C6 | `retainContextWhenHidden: true` for ≤5 panels (§17.2) | ✅ Reasonable | Note the memory math (5 × 50–80 MB) makes ">5 panels" a real UX cliff; consider a soft cap + warning |
| C7 | SSE directly from webview (§9.4) | ⚠️ Depends on 21.3 | `EventSource` cannot set custom headers, so a **token in the URL query** (or a cookie) is required for authed streams — factor into the auth design |

### 21.5 Effort re-base (honest sizing)

The original 5-phase / 10-sprint plan assumes the widget components exist.
Re-based on the findings above:

- **+ Phase 0 (1 sprint):** backend-contract reconciliation (one canonical
  spawn + port + health), CORS/token spike (§21.3), and the Path-A-vs-B
  rendering decision (§21.2). *Nothing else should start before this closes.*
- **Phase 4 ("Full Widget Parity") is the tent-pole**, not a late-stage
  polish phase. Under Path A it becomes "build + harden the generic
  renderer"; under Path B it is 60+ components and will not fit one sprint.

---

## 22. Recommendations — From "Widget Host" to Full-Fledged Trading Terminal

The current PRD delivers a **panel-docked widget dashboard inside VS Code** —
valuable, but a *terminal* implies more than dashboards. Below are grounded,
incremental recommendations that turn this into a full trading terminal,
each anchored to capabilities that already exist in this repo so they extend
rather than reinvent.

### 22.1 Command-line surface inside the terminal (reuse `cli/`)

The fork already ships a full REPL CLI ([cli/openbb_cli/](../../cli/openbb_cli/)). A real
Bloomberg-style terminal is keyboard-first. **Recommendation:** embed a
command line in the webview (or wire an integrated VS Code Task/pty) that
accepts OpenBB CLI-style commands and function-key mnemonics (F1–F12 already
map to the `pi` terminal tabs in the widget backend — see the `/pi/context/*`
"F1–F7 share this ticker" contract). This gives power users the *type-a-command*
muscle memory that distinguishes a terminal from a dashboard.

### 22.2 Watchlists & multi-symbol context (extend §13)

Symbol context today is a single "active symbol". A terminal needs
**watchlists** and **multi-symbol linking** (linked-group colors, à la
Bloomberg launchpad). **Recommendation:** promote symbol context from a
scalar to `{ activeSymbol, watchlist[], linkGroup }`, persist watchlists in
`workspaceState` (so a repo can carry its strategy universe), and let widgets
subscribe to a link-group rather than only the global active symbol.

### 22.3 Real-time streaming as a first-class transport (harden §9.4)

The PRD mentions SSE for paper status. A trading terminal lives or dies on
**live quotes/tape**. **Recommendation:** define a single streaming contract
(SSE or WS) on the chosen backend, a webview-side subscription manager
(dedup, backpressure, reconnect with jitter), and a visible connection-health
pill (reuse the `/pi/health/providers` surface). Treat "stale data" as a
loud, first-class UI state, consistent with the codebase's "loud empties"
convention in the widget backend.

### 22.4 Order & execution management (grow the paper stack toward OMS)

`tt_*` execute widgets and `pi_paper_*` already model ticket → blotter →
performance. **Recommendation:** formalize an **OMS panel**: order state
machine (new/working/filled/cancelled/rejected), keyboard order entry
(the §12 `Ctrl+Alt+B/S` is a good start — extend to a full ticket hotkey
grammar), and an audit journal view (reuse `tt_audit_journal`). Keep
paper-only for v1 (Non-Goal N2) but design the state machine so a future
broker adapter is a driver swap, not a rewrite.

### 22.5 Alerting & rules engine (make `pi_alerts_panel` actionable)

A terminal pushes information *to* you. **Recommendation:** a lightweight,
local rules engine (price/indicator/event triggers) that surfaces via VS Code
**native notifications** + status-bar badges + the `pi_alerts_panel`. Persist
rules in `.openbb/alerts.json` so they're versionable and shareable. This is
high-leverage and mostly frontend + a small poller against existing endpoints.

### 22.6 Charting depth (upgrade beyond sparklines)

The hover sparkline (§8.5) and `pi_charting` are the visual heart of a
terminal. **Recommendation:** commit to one performant charting stack for
panels (the desktop bundle already pulls charting libs), with drawing tools,
multi-pane indicators, and time-synced crosshair across linked charts. Keep
the ≤2 KB SVG sparkline for hover (per Q5) — that decision is correct.

### 22.7 Layout persistence & sharing (extend §11.2)

Exporting layouts to `.openbb/layouts/*.json` in the workspace is an
excellent, under-sold feature — it makes **layouts reviewable in PRs and
shareable across a team**. **Recommendation:** ship a small library of
committed "golden layouts" in the repo, a layout-schema version field (for
forward-compat), and a "reset to default" affordance. This is a cheap,
high-delight differentiator no competitor in §6 offers.

### 22.8 Multi-monitor / editor-group ergonomics

VS Code supports moving a webview panel to a new **editor group** and
**floating window** (VS Code 1.85+ supports "Move into new window").
**Recommendation:** make each layout a detachable floating window so a trader
can throw the Trading Desk onto a second monitor while code stays on the
first — the multi-monitor story real terminals need, delivered natively by
VS Code with no extra infrastructure.

### 22.9 Agent / notebook automation hook (unique to this fork)

The fork has a 7-phase `Analysis/` pipeline and rich notebooks. §12's
`openbb.runAnalysis` is the seed of something bigger: a terminal that can
**run an analysis and dock the result**. **Recommendation:** expose a small
command API (`openbb.run(symbol, pipeline)`) callable from notebook cells and
from the agent, returning a layout the terminal opens automatically. This is
the "code-adjacent terminal" moat — no standalone terminal can do it.

### 22.10 Prioritized build order (recommendation)

1. **Phase 0** (§21.5) — backend contract + CORS/token spike + rendering-model
   decision. *Gate everything on this.*
2. **Generic `widgets.json` renderer** (Path A) — unlocks all 60+ widgets at once.
3. **Streaming transport + connection health** (§22.3) — makes it feel *live*.
4. **Watchlists + link groups** (§22.2) and **layout sharing** (§22.7) — the
   terminal ergonomics that dashboards lack.
5. **OMS panel + hotkey order grammar** (§22.4) and **alerts** (§22.5).
6. **Charting depth** (§22.6), **multi-monitor** (§22.8), **agent hook** (§22.9).
7. Selective **Path-B bespoke widgets** where the generic renderer falls short.
