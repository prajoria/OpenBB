...
def phase1_company_profile(cfg: AnalysisConfig) -> Phase1Result:
    ...
    try:
        share_stats_df = _to_df(obb.equity.share_statistics(symbol=sym, provider=prv))
        ...
        if not np.isnan(primary):
            ...
            if (
                not np.isnan(float_shares)
                and not np.isnan(outstanding)
                and outstanding > 0
            ):
...
def phase2_fundamentals(cfg: AnalysisConfig) -> Phase2Result:
    ...
    # #1463: enrich ratios_df with columns Phase 4's quality-overlay
    # metrics (ROIC-WACC spread, Altman Z, Piotroski F) need. FMP's
    # `ratios` endpoint carries NONE of these, so Phase 4 sees NaN
    # every time without this merge.
    #
    # ROIC lives in FMP's `key-metrics` endpoint (surfaced as
    # ``obb.equity.fundamental.metrics``). Altman / Piotroski live in
    # `financial-scores` — this endpoint is exposed as a fetcher in
    # ``openbb_fmp_cached`` (``FMPCachedFinancialScoresFetcher``) but
    # NOT yet routed through the ``obb.*`` command surface, so we
    # call the fetcher directly and normalize.
    #
    # Graceful degradation: any fetch failure (endpoint down, plan
    # tier missing, unknown symbol) leaves the column absent, and
    # Phase 4's ``_latest_col`` will return NaN — same behavior as
    # today, no new exception surface.
    try:
        metrics_df = _to_df(
            obb.equity.fundamental.metrics(
                symbol=sym,
        ...
        # Both fmp_cached ratios and metrics use ``period_ending`` (not
        # ``date``). But FMP sometimes reports off-by-one dates for the
        # same fiscal period across endpoints (observed: MSFT ratios
        # 2026-07-29, metrics 2026-07-28 for the same year). So we
        # prefer ``fiscal_year`` as the merge key when both sides have
        # it; fall through to ``period_ending`` / ``date`` otherwise.
        def _merge_key(df):
            for c in ("fiscal_year", "date", "period_ending"):
                if c in df.columns:
                    return c
            return None

        m_key = _merge_key(metrics_df)
        r_key = _merge_key(ratios_df)
        roic_col = _find_col(metrics_df, ["return_on_invested_capital", "roic"])
        if (
            roic_col
            and m_key
            and r_key
            and m_key == r_key  # only merge if both sides use the same key
            and "return_on_invested_capital" not in ratios_df.columns
        ):
            m_slice = metrics_df[[m_key, roic_col]].rename(
                columns={roic_col: "return_on_invested_capital"}
            )
            # Align dtypes (fiscal_year may be int or numeric str).
            ratios_df[r_key] = ratios_df[r_key].astype(str)
            m_slice[r_key] = m_slice[r_key].astype(str)
            ratios_df = ratios_df.merge(m_slice, on=r_key, how="left")
    except Exception as exc:  # noqa: BLE001
        # Log at debug — Phase 4 still runs, quality overlays stay NaN
        # as before this fix.
        logging.getLogger(__name__).debug(
            "phase2: metrics fetch/merge for ROIC failed: %s", exc
        )

    try:
        # Direct fetcher call — `financial_scores` is not on the router.
        # Only fmp_cached is supported here; other providers would need
        # their own fetcher, so gate by provider string.
        if prv == "fmp_cached":
            import asyncio
            ...
            scores_raw = asyncio.run(
                FMPCachedFinancialScoresFetcher.aextract_data(
                    _SymbolQueryParams(symbol=sym),
                    credentials=None,
                )
            )
            # Response is a list of dicts (usually one row per symbol,
            # snapshot-shaped). Extract Altman + Piotroski and merge
            # onto ratios_df as constant columns (same value across
            # all periods — a symbol-level snapshot, not a time series).
            if scores_raw:
                row = scores_raw[0]
                # FMP returns camelCase: altmanZScore, piotroskiScore.
                altman = row.get("altmanZScore") or row.get("altman_z_score")
                pio = row.get("piotroskiScore") or row.get("piotroski_score")
                if altman is not None and "altman_z_score" not in ratios_df.columns:
                    ratios_df["altman_z_score"] = float(altman)
                if pio is not None and "piotroski_score" not in ratios_df.columns:
                    ratios_df["piotroski_score"] = float(pio)
    except Exception as exc:  # noqa: BLE001
        logging.getLogger(__name__).debug(
            "phase2: financial_scores fetch for Altman/Piotroski failed: %s", exc
        )
...
def phase4_valuation(
    ...
    if not np.isnan(fcf0) and fcf0 > 0 and not np.isnan(shares_out) and shares_out > 0:
        ...
        sensitivity_df = _dcf_sensitivity(
            fcf0, g_short, wacc, g_term, shares_out, dcf_fn=dcf_fn
        )
...
def phase5_risk(cfg: AnalysisConfig) -> Phase5Result:
    ...
    stress_scenarios = {
        ...
        "rates_up_100bps": (-0.10),  # approximate sector-neutral estimate
    ...
    if len(returns_df) >= 63:
        ...
        if sym in rolling_3m_clean.index and len(rolling_3m_clean) > 1:
            rolling_3m_rank = float(
                percentileofscore(rolling_3m_clean.tolist(), rolling_3m_clean[sym])
            )
...
def phase7_decision(
    ...
    *,
    ...
    from openbb_regime import (
        MarketRegime as _MarketRegime,
    ...
    if cfg.feature_flags.use_regime_input:
        ...
        if multiplier != 1.0:
            staged_entry = {k: v * multiplier for k, v in staged_entry.items()}
